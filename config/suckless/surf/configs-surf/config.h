...
static char *scriptfile     = "~/.config/suckless/surf/script.js";
static char *styledir       = "~/.config/suckless/surf/styles/";
static char *certdir        = "~/.config/suckless/surf/certificates/";
static char *cachedir       = "~/.config/suckless/surf/cache/";
static char *cookiefile     = "~/.config/suckless/surf/cookies.txt";
...
static int winsize[] = { 1366, 768 };		//800x600
...
#define SETPROP(r, s, p) { \
        .v = (const char *[]){ "/bin/sh", "-c", \
             ...
             "| sed \"s/^$2(STRING) = //;s/^\\\"\\(.*\\)\\\"$/\\1/\" && cut -d' ' -f1 ~/.config/suckless/surf/bookmarks)\" " \
...
#define BM_ADD(r) {\
        .v = (const char *[]){ "/bin/sh", "-c", \
             ...
             "| sed 's/.*https*:\\/\\/\\(www\\.\\)\\?//' && cat ~/.config/suckless/surf/bookmarks) " \
             "| awk '!seen[$0]++' > ~/.config/suckless/surf/bookmarks.tmp && " \
             "mv ~/.config/suckless/surf/bookmarks.tmp ~/.config/suckless/surf/bookmarks", \
             winid, r, NULL \
        } \
}
/*----------------------------------------------------------------------*/ 
...
static Key keys[] = {
	...
	// { 0,                	GDK_KEY_slash,   find,       { 0 } },
	{ 0, 				 	GDK_KEY_n,     	find,       { .i = +1 } },
	{ 0,                	GDK_KEY_b,      find,       { .i = -1 } },
	...
	{ MODKEY|GDK_CONTROL_MASK,	GDK_KEY_r,	stop,       { 0 } },
	{ 0,						GDK_KEY_r,	reload,     { .i = 0 } },

	// move foreward/backward page 
	{ GDK_SHIFT_MASK,			GDK_KEY_l,	navigate,   { .i = +1 } },
	{ GDK_SHIFT_MASK,			GDK_KEY_h,	navigate,   { .i = -1 } },

	// bookmarks
	{ GDK_SHIFT_MASK, 			GDK_KEY_b, 	spawn,      BM_ADD("_SURF_URI") },
	/*---------------------------------------------------------------------------*/
	/* 			     end of PAGE and NAVIGATION									 */
	/*---------------------------------------------------------------------------*/
	...
	/*---------------------------------------------------------------------------*/
	/* 			end of CLIPBOARD and COPY/PASTE									 */
	/*---------------------------------------------------------------------------*/
